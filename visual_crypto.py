...
def get_options():
    ...
    parser.add_argument(
        ...
        "-m",
        required=True,
        metavar="MESSAGE_IMAGE_FILE_PATH",
        help="message image",
    )
    parser.add_argument(
        "--secret",
        "-s",
        metavar="SECRET_IMAGE_FILE_PATH",
        default="secret.png",
        help="secret image (will be created if it does not exist)",
    )
    parser.add_argument(
        "--ciphered",
        "-c",
        metavar="CIPHERED_IMAGE_FILE_PATH",
        default="ciphered.png",
        help="ciphered image (to be generated)",
    )
    parser.add_argument(
        "--resize",
        "-r",
        metavar="WIDTH,HEIGHT",
        help="resize message image (defaults to message image size). If width or height is larger than that of the secret image, the secret image will be enlarged appropriately.",
    )
    parser.add_argument(
        "--prepared_message",
        "-p",
        metavar="PREPARED_MESSAGE_IMAGE_FILE_PATH",
        help="if present, the prepared message image is saved to this path",
    )
...
def save_with_transparency(image, path):
...
def main():
    ...
    if os.path.isfile(args.secret):
        try:
            ...
            logging.fatal(
                "I/O error while loading secret image '%s' (%s)" % (args.secret, str(e))
            )
            sys.exit(2)
    else:
    ...
    if save_secret:
        ...
        try:
            save_with_transparency(secret_image, args.secret)
        except IOError as e:
            logging.error(
                "I/O error while saving secret image '%s' (%s)" % (args.secret, str(e))
            )

    if args.prepared_message:
        logging.debug("Saving prepared message image '%s'" % (args.prepared_message))
        try:
            save_with_transparency(prepared_image, args.prepared_message)

        except IOError as e:
            logging.error(
                "I/O error while saving prepared message image '%s' (%s)"
                % (args.prepared_message, str(e))
            )

    try:
        save_with_transparency(ciphered_image, args.ciphered)
    except IOError as e:
        logging.fatal(
            "I/O error while saving ciphered image '%s' (%s)" % (args.ciphered, str(e))
        )
        sys.exit(3)
